...
window.loading = {
    html:'<div id="loadingDiv" style="position:fixed;background:url(../img/loading.gif) no-repeat 50% 50%;width:32px;height:32px;z-index:2016"></div>',
    _pos: function() {
        var a = document,
            b = a.documentElement,
            g = a.body,
            e, d, f;
        a = g.scrollTop, g = g.scrollLeft, e = b.clientWidth, b = b.clientHeight, d = 32, f = 32;
        return {top: a + (b - f) / 2,left: g + (e - d) / 2}
    },
    show:function () {
        var pos = this._pos();
        $(this.html).css(pos).appendTo($('body'));
    },
    hide:function () {
        $('#loadingDiv').remove();
    }
